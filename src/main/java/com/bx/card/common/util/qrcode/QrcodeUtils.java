...
public class QrcodeUtils {
	...
	private static final int DEFAULT_LENGTH = 300;// 生成二维码的默认边长，因为是正方形的，所以高度和宽度一致
	...
	public byte[] createQrcode(MatrixToBgImageConfig config) {
		try {
...
//
//			} else {
			InputStream headimgStream = null;
			if(StringUtils.isNotEmpty(config.getHeadimgUrl())){
				String logFileFullName = qrCodeConfig.getLogoRepository()+File.separator+config.getHeadimgUrl();
				headimgStream = new FileInputStream(logFileFullName);
			}else{
				headimgStream = Thread.currentThread().getContextClassLoader()
						.getResourceAsStream(Strings.toString(config.getHeadimgUrl(), MatrixToBgImageConfig.DEFAULT_HEADIMGURL));
			}
...
//			}
	...
	private static void increasingImage(BufferedImage image, String format, String imagePath, File bgFile,
		...
		try {
			...
			g.drawString(config.getRealname()+"-"+ Strings.toString(config.getOrg()), config.getRealname_x(), config.getRealname_y());
			if(StringUtils.isNotEmpty(config.getJob()))
				g.drawString(config.getJob(), config.getJob_x(), config.getJob_y());


			g.setColor(new Color(33,45,23));// 文字颜色
			Font font2 = new Font("宋体", Font.BOLD, 20);
			g.setFont(font2);

			if(StringUtils.isNotEmpty(config.getAdd()))
				g.drawString(config.getAdd(), config.getAdd_x(), config.getAdd_y());
			if(StringUtils.isNotEmpty(config.getPhone()))
				g.drawString(config.getPhone(), config.getPhone_x(), config.getPhone_y());
			if(StringUtils.isNotEmpty(config.getTel()))
				g.drawString(config.getTel(), config.getTel_x(), config.getTel_y());
			if(StringUtils.isNotEmpty(config.getFax()))
				g.drawString(config.getFax(), config.getFax_x(), config.getFax_y());
